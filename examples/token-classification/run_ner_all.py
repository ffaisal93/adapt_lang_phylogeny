...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        #      example['xpos'] = [str(x) for x in example['xpos']] 
        #      return example
        # for x in raw_datasets:
        #     raw_datasets[x] = raw_datasets[x].map(remove_none)
        # raw_datasets = datasets.DatasetDict({"train":raw_datasets[0],"validation":raw_datasets[1],"test":raw_datasets[2]})
    else:
        data_files = {}
    ...
    task_name = "pos_" + data_args.dataset_config_name
    # Setup adapters
    ...
    def get_dataset(langs):
        raw_datasets = load_dataset("../adapter-transformers/examples/dependency-parsing/universal_dependencies.py", langs,
                               split=['test'], cache_dir=model_args.cache_dir)
        raw_datasets = datasets.DatasetDict({"train":raw_datasets[0],"test":raw_datasets[0]})
        # def remove_none(example):
        #      example['xpos'] = [str(x) for x in example['xpos']] 
        #      return example
        # for x in raw_datasets:
    ...
    if training_args.do_predict_all:
        task_adapters =[
            'et_ewt',
            'et_ewt_lang',
            'et_ewt_joint_lang_family_region',
        ]
        # task_adapters =[
        #     'en_ewt',
        #     'en_ewt_lang',
        ...
        train_task_lang='pos_en_ewt'
        train_task_lang='pos_et_ewt'

        task_path='/scratch/ffaisal/run_mlm/experiments/pos/'
        ...
        def  do_prediction_all(text,adapter_list):
            ...
            logger.info("%s,%s,%s,%s,%s\n" % (text,adapter_list[-1], lang, metrics['predict_f1'], metrics['predict_accuracy']))
